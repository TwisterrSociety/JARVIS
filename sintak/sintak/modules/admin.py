...
def tampilkan_list(data, fields, title):
    print(f"\n=== {title} ===")
    if not data:
    ...
    for i, row in enumerate(data, 1):
        info = " | ".join([f"{f}: {row.get(f,'')}" for f in fields])
        print(f"{i}. {info}")
    print("-"*50)

# Contoh integrasi search & sort pada menu mahasiswa
def kelola_mahasiswa():
    mhs = read_txt("data/mahasiswa.txt")
    print("\n1. Search mahasiswa\n2. Sort mahasiswa (nama)\n3. Tampilkan semua")
    pilih = input("Pilih menu: ").strip()
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(mhs, ["nama", "nim"], keyword)
        tampilkan_list(hasil, ["nama", "nim"], "Hasil Pencarian Mahasiswa")
    elif pilih == "2":
        hasil = sort_data(mhs, "nama")
        tampilkan_list(hasil, ["nama", "nim"], "Mahasiswa Terurut Nama")
    else:
        tampilkan_list(mhs, ["nama", "nim"], "Daftar Mahasiswa")
...
def kelola_nilai():
    ...
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(nilai, ["nim", "matkul"], keyword)
        tampilkan_list_nilai(hasil)
    elif pilih == "2":
        hasil = sort_data(nilai, "matkul")
        tampilkan_list_nilai(hasil)
    else:
        tampilkan_list_nilai(nilai)
...
def kelola_absensi():
    ...
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(absensi, ["nim", "matkul", "tanggal"], keyword)
        tampilkan_list_absensi(hasil)
    elif pilih == "2":
        hasil = sort_data(absensi, "tanggal")
        tampilkan_list_absensi(hasil)
    else:
        tampilkan_list_absensi(absensi)
...
def tampilkan_list_spp(spp):
    print("\n=== Daftar SPP ===")
    if not spp:
        print("Tidak ada data SPP ditemukan.")
        print("-"*50)
        return
    print("-"*50)
    for i, s in enumerate(spp, 1):
        print(f"{i}. {s.get('nim','')} | Tahun: {s.get('tahun','')} | Semester: {s.get('semester','')} | Nominal: {s.get('nominal','')} | Status: {s.get('status','')}")
    print("-"*50)

def kelola_spp():
    spp = read_txt("data/spp.txt")
    print("\n1. Tambah SPP\n2. Search SPP\n3. Sort SPP (tahun)\n4. Tampilkan semua")
    pilih = input("Pilih menu: ").strip()
    if pilih == "1":
        nim = input("NIM: ").strip()
        ...
        semester = input("Semester: ").strip()
        nominal = input("Nominal (angka): Rp").strip()
        status = input("Status (Lunas/Belum Lunas): ").strip()
        try:
            nominal = float(nominal)
        except:
            print("Nominal harus angka!"); return
        if not nim or not tahun or not semester or not status:
            print("Data tidak boleh kosong!"); return
        spp.append({"nim": nim, "tahun": tahun, "semester": semester, "nominal": nominal, "status": status})
        write_txt("data/spp.txt", spp)
        print("SPP berhasil ditambah.")
    elif pilih == "2":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(spp, ["nim", "tahun", "semester", "status"], keyword)
        tampilkan_list_spp(hasil)
    elif pilih == "3":
        hasil = sort_data(spp, "tahun")
        tampilkan_list_spp(hasil)
    else:
        tampilkan_list_spp(spp)
...
def kelola_notifikasi():
    notif = read_txt("data/notifications.txt")
    print("\n1. Search notifikasi\n2. Sort notifikasi (date)\n3. Tampilkan semua")
    pilih = input("Pilih menu: ").strip()
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(notif, ["title", "message"], keyword)
        tampilkan_list_notifikasi(hasil)
    elif pilih == "2":
        hasil = sort_data(notif, "date")
        tampilkan_list_notifikasi(hasil)
    else:
        tampilkan_list_notifikasi(notif)
