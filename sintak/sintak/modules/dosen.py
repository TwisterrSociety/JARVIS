...
def input_nilai(id_dosen):
    ...
    # Validasi: cek kode atau nama matkul (case-insensitive)
    mk_lower = mk.lower()
    allowed_lower = [str(x).lower() for x in allowed]
    if mk_lower not in allowed_lower:
    ...
    # Tampilkan data terbaru setelah input
    rows = read_txt('data/nilai.txt')
    print("\nData nilai terbaru:")
    for r in rows[-5:]:
        print(f"{r.get('nim','')} | {r.get('matkul','')} | {r.get('nilai','')} | SKS:{r.get('sks','-')} | Semester:{r.get('semester','-')} | Dosen:{r.get('dosen_id','-')}")
    pause()
...
def view_attendance_results(id_dosen):
    ...
    # Filter sesi yang dibuat oleh dosen ini ATAU sesuai matkul yang dia ampu
    allowed = allowed_matkul_for_dosen(id_dosen)
    my_sessions = [
        s for s in sessions
        if s.get('dosen_id') == id_dosen or s.get('mata_kuliah') in allowed
    ]
    ...
    print("-" * 60)
    sid = input("Masukkan ID sesi untuk melihat rekap (Enter untuk batal): ").strip()
    if not sid:
        return

    # Ambil record dari attendance_records.txt
    records = read_txt('data/attendance_records.txt')

    # Support berbagai kemungkinan key: session_id OR session
    recs = [
        r for r in records
        if r.get('session_id') == sid or r.get('session') == sid
    ]

    print("\n=== HASIL ABSENSI ===")
    if not recs:
        print("⚠️ Belum ada mahasiswa yang absensi pada sesi ini.")
        return

    # Hitung rekap hadir / izin / alpha
    hadir = sum(1 for r in recs if str(r.get("status","")).lower() == "hadir")
    izin  = sum(1 for r in recs if "izin"  in str(r.get("status","")).lower())
    alfa  = sum(1 for r in recs if "alpha" in str(r.get("status","")).lower() or "alfa" in str(r.get("status","")).lower())

    print(f"Rekap: Hadir: {hadir} | Izin: {izin} | Alfa: {alfa}")
    print("-" * 60)

    # Tampilkan record lengkap
    print("NIM | Status | Timestamp | Keterangan")
    print("-"*60)
    for r in recs:
        print(
            f"{r.get('nim')} | "
...
def tampilkan_list_nilai(nilai):
    print("\n=== Daftar Nilai ===")
    if not nilai:
        print("Tidak ada nilai ditemukan.")
        print("-"*50)
        return
    print("-"*50)
    for i, n in enumerate(nilai, 1):
        print(f"{i}. {n.get('nim','')} | {n.get('matkul','')} | Nilai: {n.get('nilai','')}")
    print("-"*50)

def kelola_nilai_dosen(id_dosen):
    nilai = read_txt("data/nilai.txt")
    nilai_saya = [n for n in nilai if n.get("dosen_id") == id_dosen]
    print("\n1. Search nilai\n2. Sort nilai (matkul)\n3. Tampilkan semua")
    pilih = input("Pilih menu: ").strip()
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(nilai_saya, ["nim", "matkul"], keyword)
        tampilkan_list_nilai(hasil)
    elif pilih == "2":
        hasil = sort_data(nilai_saya, "matkul")
        tampilkan_list_nilai(hasil)
    else:
        tampilkan_list_nilai(nilai_saya)

def tampilkan_list_absensi(absensi):
    print("\n=== Daftar Absensi ===")
    if not absensi:
        print("Tidak ada absensi ditemukan.")
        print("-"*50)
        return
    print("-"*50)
    for i, a in enumerate(absensi, 1):
        print(f"{i}. {a.get('nim','')} | {a.get('matkul','')} | {a.get('tanggal','')} | Status: {a.get('status','')}")
    print("-"*50)

def kelola_absensi_dosen(id_dosen):
    absensi = read_txt("data/attendance_records.txt")
    absensi_saya = [a for a in absensi if a.get("dosen_id") == id_dosen]
    print("\n1. Search absensi\n2. Sort absensi (tanggal)\n3. Tampilkan semua")
    pilih = input("Pilih menu: ").strip()
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(absensi_saya, ["nim", "matkul", "tanggal"], keyword)
        tampilkan_list_absensi(hasil)
    elif pilih == "2":
        hasil = sort_data(absensi_saya, "tanggal")
        tampilkan_list_absensi(hasil)
    else:
        tampilkan_list_absensi(absensi_saya)

def tampilkan_list_notifikasi(notif):
    print("\n=== Notifikasi Dosen ===")
    if not notif:
    ...
    for i, n in enumerate(notif, 1):
        print(f"{i}. {n.get('title','')} | {n.get('message','')} | {n.get('date','')}")
    print("-"*50)

def tampilkan_notifikasi_dosen(id_dosen):
    notif_raw = read_txt("data/notifications.txt")
    notif = []
    for n in notif_raw:
        # Handle CSV line
        ...
        # Handle JSON line
            ...
            role = n.get("role", n.get("role_or_all", ""))
            title = n.get("title", "")
            message = n.get("message", n.get("pesan", ""))
            date = n.get("date", n.get("tanggal", ""))
            notif.append({"role": role, "title": title, "message": message, "date": date})
    notif_dosen = [n for n in notif if n["role"] in ["dosen", "all"]]
    print("\n1. Search notifikasi\n2. Sort notifikasi (date)\n3. Tampilkan semua")
    pilih = input("Pilih menu: ").strip()
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(notif_dosen, ["title", "message"], keyword)
        tampilkan_list_notifikasi(hasil)
    elif pilih == "2":
        hasil = sort_data(notif_dosen, "date")
        tampilkan_list_notifikasi(hasil)
    else:
        tampilkan_list_notifikasi(notif_dosen)
...
def tampilkan_list_jadwal(jadwal):
    print("\n=== Jadwal Mata Kuliah ===")
    if not jadwal:
        ...
        return
    print("-"*50)
    for i, j in enumerate(jadwal, 1):
        print(f"{i}. {j.get('nama','')} | Hari: {j.get('hari','')} | Jam: {j.get('jam','')} | Ruang: {j.get('ruang','')}")
    print("-"*50)

def lihat_jadwal_semua(id_dosen):
    jadwal = read_txt("data/matkul_schedule.txt")
    jadwal_saya = [j for j in jadwal if j.get("dosen_id") == id_dosen]
    print("\n1. Search jadwal\n2. Sort jadwal (hari)\n3. Tampilkan semua")
    pilih = input("Pilih menu: ").strip()
    if pilih == "1":
        keyword = input("Kata kunci: ").strip()
        hasil = multisearch(jadwal_saya, ["nama", "hari", "jam"], keyword)
        tampilkan_list_jadwal(hasil)
    elif pilih == "2":
        hasil = sort_data(jadwal_saya, "hari")
        tampilkan_list_jadwal(hasil)
    else:
        tampilkan_list_jadwal(jadwal_saya)
    pause()
